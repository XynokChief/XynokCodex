...
namespace XynokConvention.Data.BigData
{
    /// <summary>
    /// use for number over E+308 
    /// </summary>
    ...
    public struct BigDouble : IFormattable, IComparable, IComparable<BigDouble>, IEquatable<BigDouble>
    {
        ...
        public static BigDouble Remainder(BigDouble a, BigDouble b)
        {
            return a - b * Truncate(a / b);
        }

        public static BigDouble RemainderDecimal(BigDouble a, BigDouble b)
        {
    ...
    public static class BigDoubleExtensions
    {
        ...
        public static string GetStringRepresentation(this BigDouble number, int decimalCount = 3)
        {
            string[] suffixes = new[]
            {
        ...
        public static (string, string) GetRepresentation(this BigDouble number, string[] suffixes, int decimalCount = 3)
        {
            ...
            for (int i = 1;; i += 1)
            {
                BigDouble val = BigDouble.Pow(1000, i);

                if (i > suffixes.Length) break;

                if (val > number)
                {
                    var ratio = BigDouble.Pow(1000, i - 1);
                    var integer = (number / ratio).Floor();
                    var remainder = BigDouble.RemainderDecimal(number, ratio);

                    var remainderStr = remainder.ToString();
                    var remainderTxt = remainderStr;
                    if (remainderStr.Contains("."))
                    {
                         remainderTxt = remainderStr.Split('.')[1];
                        remainderTxt = remainderTxt.Length <= validDecimalCount
                            ? remainderTxt
                            : remainderTxt.Substring(0, validDecimalCount);
                    }
                    ...
                    var integerTxt = $"{integer.ToString()}{thapPhan}";
